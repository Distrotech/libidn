...
main (int argc, char *argv[])
{
  ...
  do
      ...
      if (args_info.idna_to_ascii_given)
	{
	  ...
	  if (rc != IDNA_SUCCESS)
	    {
	      fprintf (stderr, "%s: idna_to_ascii_4z() failed "
		       "with error %d.\n", argv[0], rc);
	      return 1;
	    }
	  ...
	  if (args_info.tld_flag)
	    {
	      ...
	      if (rc != IDNA_SUCCESS)
		{
		  fprintf (stderr, "%s: TLD idna_to_unicode_8z8z() failed "
			   "with error %d.\n", argv[0], rc);
		  return 1;
		}

	      rc = tld_check_8z (r, &errpos, NULL);
	      if (rc == TLD_INVALID)
		{
	      ...
	      if (rc != TLD_SUCCESS)
		{
		  fprintf (stderr, "%s: tld_check_8z failed with error %d.\n",
			   argv[0], rc);
		  free (r);
		  return 1;
		}
      ...
      if (args_info.idna_to_unicode_given)
	{
	  ...
	  if (rc != IDNA_SUCCESS)
	    {
	      fprintf (stderr, "%s: idna_to_unicode_8z4z() "
		       "failed with error %d.\n", argv[0], rc);
	      return 1;
	    }
	  ...
	  if (args_info.tld_flag)
	      ...
	      if (rc == TLD_INVALID)
		{
	      ...
	      if (rc != TLD_SUCCESS)
		{
		  fprintf (stderr, "%s: tld_check_4z failed with error %d.\n",
			   argv[0], rc);
		  free (q);
		  return 1;
		}
